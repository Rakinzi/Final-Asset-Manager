...
class Validate
{
    ...
    private $user = 'elqppdzg_silver';
    private $pass = '45m*Silver';
    private $db = 'elqppdzg_assets_management';
    ...
    public function registerAsset($userID, $regNumber, $deviceName, $deviceModel, $serialNumber, $signature)
    {
        ...
        if ($this->checkAsset($deviceName, $deviceModel, $serialNumber) == 'Missing') {
            ...
            if ($stmt->affected_rows > 0) {
                $id =  $stmt->insert_id;
                $url = 'http://templates.merza.co.zw/card.php?aid=' . $id . '';
                ...
                $qr_link = 'http://templates.merza.co.zw/verify.php?qrcode=' . $qr_id . '.jpg';
...
$output = '<img><br>
    <h3>Dear ' . $username . '</h3>
    <p>Please click on the link below to download:</p>
